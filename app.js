...
function handleDragMove(clientY) {
  ...
  // 미세한 탭 오동작 방지를 위해 3px 이상 세로로 이동했을 때만 드래그 시작으로 간주하고 슬라이더 표출
...
// 화면을 터치하거나 마우스를 움직일 때마다 하단 상태 표시줄을 일시적으로 보여줌
window.addEventListener('touchstart', showStatusBarTemporarily, { passive: true });
...
function onFirstUserInteraction() {
  ...
  // 전체화면 요청 (태블릿 상단 알림 바 및 하단 내비게이션 바 숨김)
  try {
    var docEl = document.documentElement;
    if (docEl.requestFullscreen) {
      docEl.requestFullscreen();
    } else if (docEl.webkitRequestFullscreen) { // iOS 사파리 및 구형 크롬 대응
